...
async function claude(env, system, user, maxTokens = 900) {
  if (!env.ANTHROPIC_API_KEY) throw new Error("ANTHROPIC_API_KEY não configurada no servidor.");
  const r = await fetch("https://api.anthropic.com/v1/messages", {
    method: "POST",
    headers: { "x-api-key": env.ANTHROPIC_API_KEY, "anthropic-version": "2023-06-01", "content-type": "application/json" },
    body: JSON.stringify({ model: env.MC_MODEL || "claude-sonnet-4-6", max_tokens: maxTokens, system, messages: [{ role: "user", content: user }] }),
  });
  if (!r.ok) throw new Error("Falha na IA (" + r.status + ")");
  const d = await r.json();
  return (d.content || []).map((p) => p.text || "").join("");
}
async function claudeJson(env, system, user, maxTokens = 1000) {
  const txt = await claude(env, system + " Responda APENAS com JSON válido, sem texto fora do JSON.", user, maxTokens);
...
async function overview(env, u) {
  ...
  return { ssi: s || {}, profile: pr || {}, threads: await listThreads(env, u), ai_enabled: !!env.ANTHROPIC_API_KEY };
}
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (p === "/api/ai/improve") {
      ...
      let out; try { out = await claudeJson(env, sys, usr, 700); } catch (e) { return json({ error: e.message }, 502); }
    ...
    if (p === "/api/ai/jobfit") {
      ...
      let out; try { out = await claudeJson(env, sys, usr, 1100); } catch (e) { return json({ error: e.message }, 502); }
    ...
    if (p === "/api/ai/post") {
      ...
      let post; try { post = await claude(env, sys, usr, 700); } catch (e) { return json({ error: e.message }, 502); }
      return json({ post });
    }
